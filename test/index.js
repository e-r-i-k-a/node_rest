...
const {TEST_VALID_ADDRESS, TEST_INVALID_ADDRESS} = require('../utility');
...
describe('GET/api/{key}/{value}', () => {
  it('lists all the stored address records for a given state and country', async () => {
    ...
    await request(app).get(`/api/country/${TEST_VALID_ADDRESS.country}`)
    .expect(res => {
      const {body} = res;
      expect(body.length === 1);
      expect(body.country === TEST_VALID_ADDRESS.country);
    });
    await request(app).post('/api').send({country: TEST_VALID_ADDRESS.country});
    await request(app).get(`/api/country/${TEST_VALID_ADDRESS.country}`)
    .expect(res => {
      const {body} = res;
      expect(body.length === 2);
      assert.equal(body[0].country, body[1].country, TEST_VALID_ADDRESS.country);
    });
    await request(app).post('/api').send({country: 'another country name'});
    await request(app).get(`/api/country/${TEST_VALID_ADDRESS.country}`)
    .expect(res => expect(res.body.length === 2));
    await request(app).get(`/api/state/${TEST_VALID_ADDRESS.state}`)
    .expect(res => {
      const {body} = res;
      expect(body.length === 1);
      assert.equal(body[0].state, TEST_VALID_ADDRESS.state);
    });
  });
});
...
describe('POST/api', () => {
  ...
  it('checks that the state is valid for the country - POST', async () => {
    await request(app).post('/api').send(TEST_INVALID_ADDRESS).expect(422)
    .expect(res => expect(res.error === 'Invalid state and country combination.'));
  });
});

describe('PUT/api', () => {
  it ('fails with an invalid ID', async () => {
    await request(app).put('/api/fakeId').send(TEST_VALID_ADDRESS).expect(500)
